...
```{r}
library(readr)
data <- read_csv("https://raw.githubusercontent.com/opencampus-sh/einfuehrung-in-data-science-und-ml/main/umsatzdaten_gekuerzt.csv")
```
...
```{r}
  mod <- lm(Umsatz ~ as.factor(Warengruppe), train_data)
  summary(mod)
...
```{r}
library(glmnet)
mod_glm <- glmnet(as.matrix(train_data[c('sqft_lot15', 'condition')]), train_data$price)
mod_glm
